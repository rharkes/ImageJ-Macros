...
// Dont forget to set the units in Analyse->Calibrate
  ...
  function addCalibrationBar() {
     ...
     stackID = getImageID();
     setSlice(1);
     run("Calibration Bar...", "location="+location+" fill="+fil+" label="+label+" number="+number+" decimal="+decimal+" font="+font+" zoom="+zoom+" show");
     barID = getImageID();
     for (n=2; n<=nS; n++) {
        showProgress(n, nS);
        selectImage(stackID);
